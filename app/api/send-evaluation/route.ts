...
export async function POST(request: Request) {
  try {
    const { to, name, po, evaluationUrl, workspace } = await request.json();
    ...
    if (!to || !evaluationUrl || !po?.poNumber) return NextResponse.json({ ok: false, message: "Missing recipient or evaluation details." }, { status: 400 });
    const subject = `[${workspace?.name || "SISC"}] Supplier Evaluation Request — ${po.poNumber}`;
    const html = `<!doctype html><html><body style="margin:0;padding:24px;background:#f4f5fb;font-family:Arial,Helvetica,sans-serif;color:#111827"><div style="max-width:620px;margin:auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e5e7eb"><div style="background:#102e24;padding:24px 28px;color:#fff"><div style="font-size:12px;opacity:.72;text-transform:uppercase;letter-spacing:1px">${workspace?.name || "Southville International School and Colleges"}</div><div style="font-size:22px;font-weight:700;margin-top:5px">Supplier Evaluation Request</div></div><div style="padding:28px"><p>Hello <strong>${name || po.requisitioner || "Requisitioner"}</strong>,</p><p>Please review the generated Purchase Order below and complete the supplier evaluation for this transaction.</p><div style="margin:18px 0;padding:16px;border:1px solid #e5e7eb;border-radius:10px;background:#f8fafc"><div><strong>PO Number:</strong> ${po.poNumber}</div><div><strong>Supplier:</strong> ${po.vendorName || "—"}</div><div><strong>PRF No.:</strong> ${po.prfNumber || "—"}</div><div><strong>Total:</strong> Php ${Number(po.total || 0).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</div></div><div style="text-align:center;margin:28px 0"><a href="${evaluationUrl}" style="display:inline-block;background:#2a895b;color:#fff;text-decoration:none;padding:13px 24px;border-radius:9px;font-weight:700">Open PO & Complete Evaluation</a></div><p style="font-size:12px;color:#6b7280">The evaluation is saved to the purchasing system and can only be submitted once using this unique link.</p></div></div></body></html>`;
