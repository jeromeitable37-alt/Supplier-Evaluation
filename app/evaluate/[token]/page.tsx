...
export default function EvaluationPage() {
  ...
  return <div className="public-eval-shell"><div className="public-eval-wrap">
    ...
    <section className="public-po-summary"><div><span>PO Number</span><strong>{po.poNumber}</strong></div><div><span>Supplier</span><strong>{po.vendorName || "—"}</strong></div><div><span>PRF No.</span><strong>{po.prfNumber || "—"}</strong></div><div><span>Delivery Date</span><strong>{po.expectedDate || "—"}</strong></div><div><span>Total Amount</span><strong>Php {displayTotal.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></div><div><span>Requisitioner</span><strong>{po.requisitioner || link.requisitionerName || "—"}</strong></div></section>
    ...
    <section className="public-po-pricing" style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, marginBottom: 18, overflow: "hidden" }}>
